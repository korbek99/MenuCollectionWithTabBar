...
class ProductViewController: UIViewController {
    ...
    let productsNote: [Product] = productList.filter { $0.typeDevice == 1 }
    let productsDev: [Product] = productList.filter { $0.typeDevice == 2 }
...
extension ProductViewController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return collectionView == featuredCollectionView ? productsNote.count : productsDev.count
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView == featuredCollectionView {
            ...
            let item = productsNote[indexPath.item]
            let model = FeaturedModelViewCell(title: item.itemName, urlToImage: item.photo)
            cell.configureView(model: model)
            return cell
        } else {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: ImageCollectionViewCell.identifier, for: indexPath) as! ImageCollectionViewCell
            let item = productsDev[indexPath.item]
            let model = ImageModelViewCell(title: item.itemName, urlToImage: item.photo)
            cell.configureView(model: model)
            return cell
        }
    }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
            if collectionView == featuredCollectionView {
                let item = productsNote[indexPath.item]
                showDetailViewController(imageURL: item.photo, title: item.itemName, desc: item.description)
            } else {
                let item = productsDev[indexPath.item]
                showDetailViewController(imageURL: item.photo, title: item.itemName, desc: item.description)
            }
        ...
        private func showDetailViewController(imageURL: String, title: String ,  desc: String) {
